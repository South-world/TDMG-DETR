...
from engine.extre_module.custom_nn.transformer.TDMSA import GConv3,GConv4
import logging
...
class DualGConvBNAct(nn.Module):
    def __init__(
            self,
            in_chs,
            out_chs,
        ...
        self.conv2 = GConv4(in_chs,out_chs,act_layer)
        self.bn = nn.BatchNorm2d(out_chs)
        if self.use_act:
            self.act = nn.ReLU()
        else:
...
#     """
#     DualHGNetv2 的 Stem Block，作为网络的输入处理模块。
#
#     主要作用：
#     1. 进行初步特征提取（stem1）。
